...
from nolearn.lasagne import BatchIterator
from utilities import (AdjustVariable,FlipBatchIterator)
'''
...
if __name__ == '__main__':
    ...
    yy = pd.DataFrame(y)
    yy.columns = ['a','b','c','d']
    idx_d = yy[yy['d'] == 1].index.tolist()[:1200]
    idx_c = yy[yy['c'] == 1].index.tolist()[:1200]
    idx_b = yy[yy['b'] == 1].index.tolist()[:1200]
    idx_a = yy[yy['a'] == 1].index.tolist()[:1200]
    X = X[idx_a+idx_b+idx_c+idx_d]
    y = y[idx_a+idx_b+idx_c+idx_d]
    #X = X[idx_a+idx_d]
    #y = y[idx_a+idx_d]

    #X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.33, random_state=13)
    skf = StratifiedKFold([str(x) for x in y], n_folds=8,shuffle=True)
    ...
    nnet = NeuralNet(
        ...
        pool4_pool_size=(2, 2),
        ##
        #conv50_num_filters=580, 
        #conv50_nonlinearity=rectify,
        #conv50_filter_size=(3, 3), 
        ##
        #conv51_num_filters=580, 
        #conv51_nonlinearity=rectify,
        #conv51_filter_size=(3, 3), 
        ##
        #conv52_num_filters=580, 
        #conv52_nonlinearity=rectify,
        #conv52_filter_size=(3, 3), 
        ##
        #pool5_pool_size=(2, 2),
        ##
        ...
        #update_learning_rate=theano.shared(np.float32(0.0000005)),
